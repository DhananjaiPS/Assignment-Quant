...
*   **Frontend & API App (Vercel)**: [https://quantacus-intelligence.vercel.app](https://quantacus-intelligence.vercel.app) *(Simulated)*
*   *(Note: True background processing requires a long-running Node instance. In a production environment, the `tsx` worker would be hosted on a persistent container service like Railway, Render, or AWS ECS, rather than Vercel Serverless Functions).*
