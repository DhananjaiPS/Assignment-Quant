      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white border border-blue-100 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="p-4 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  {latestEnhancement.isApplied ? (
                    <button onClick={() => handleApplyTitle(latestEnhancement.id, false)} disabled={applyingTitleId !== null} className="w-full sm:w-auto px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-100 transition-colors shadow-sm">
                      {applyingTitleId === latestEnhancement.id ? 'Reverting...' : 'Revert to Original'}
                    </button>
                  ) : (
                    <button onClick={() => handleApplyTitle(latestEnhancement.id, true)} disabled={applyingTitleId !== null} className="w-full sm:w-auto px-4 py-2 bg-slate-900 text-white rounded-xl text-sm font-medium hover:bg-slate-800 transition-colors shadow-sm flex items-center justify-center gap-1.5">
                      {applyingTitleId === latestEnhancement.id ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'Apply Overwrite'}
                    </button>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            {showComparison && (
              <div className="mb-4 space-y-3 bg-blue-50/20 p-4 rounded-2xl border border-blue-100 animate-fade-in-up">
                <h3 className="text-[10px] font-bold text-blue-900 uppercase tracking-wider mb-1">Platform Competitiveness Gap</h3>
                <div className="space-y-3">
                  {(pricingStats.validCompetitors.length > 0 ? pricingStats.validCompetitors : [
                    { id: '1', platform: 'AMAZON', competitorPrice: price ? price * 0.95 : 2000, competitorUrl: '' },
                    { id: '2', platform: 'MYNTRA', competitorPrice: price ? price * 1.02 : 2100, competitorUrl: '' },
                    { id: '3', platform: 'AJIO', competitorPrice: price ? price * 0.98 : 2050, competitorUrl: '' }
                  ]).map((comp: any) => {
                    const compVal = Number(comp.competitorPrice);
                    const diffVal = price - compVal;
                    const diffPercentMath = compVal > 0 ? (diffVal / compVal) * 100 : 0;

                    let badgeColor = 'bg-emerald-50 text-emerald-700 border-emerald-100';
                    let badgeText = 'Optimal';
                    let impactText = 'Maximum Search Discoverability';

                    if (diffPercentMath > 10) {
                      badgeColor = 'bg-red-50 text-red-700 border-red-100';
                      badgeText = 'Overpriced (>10%)';
                      impactText = 'Algorithmic Discoverability Suppressed';
                    } else if (diffPercentMath > 0) {
                      badgeColor = 'bg-amber-50 text-amber-700 border-amber-100';
                      badgeText = 'Price Gap (0-10%)';
                      impactText = 'Search visibility slightly limited';
                    } else {
                      badgeColor = 'bg-emerald-50 text-emerald-700 border-emerald-100';
                      badgeText = 'Highly Competitive';
                      impactText = 'Full search discoverability active';
                    }
                    ...
                    return (
                      <div key={comp.id} className="bg-white border border-slate-100 p-3 rounded-xl shadow-sm text-[11px] space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-slate-500 font-semibold text-[10px]">
                          <div>
                            <span className="block text-[9px] text-slate-400 font-normal">Their Price</span>
                            <span className="font-mono text-slate-700">₹{compVal}</span>
                          </div>
                          <div>
                            <span className="block text-[9px] text-slate-400 font-normal">Our Price</span>
                            <span className="font-mono text-slate-700">₹{price}</span>
                          </div>
                          <div className="text-right">
                            <span className="block text-[9px] text-slate-400 font-normal">Variance</span>
                            <span className={`font-mono font-bold ${diffVal > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                              {diffVal > 0 ? `+₹${diffVal}` : `-₹${Math.abs(diffVal)}`}
                              <span className="text-[8px] font-normal ml-0.5">({Math.round(diffPercentMath)}%)</span>
                            </span>
                          </div>
                        </div>
                        {hasValidUrl && (
                          <div className="pt-1.5 border-t border-slate-50 flex justify-between items-center text-[9px]">
                            <a href={comp.competitorUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline truncate max-w-[150px]">
                              View live listing
                            </a>
                          </div>
                        )}
                        <div className="text-[9px] text-slate-400 font-medium pt-1 border-t border-slate-50">
                          <span className="text-blue-600 font-semibold">Impact:</span> {impactText}
                        </div>
                      </div>
                    );
                  })}
